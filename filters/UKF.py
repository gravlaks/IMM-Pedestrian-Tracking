...
class UKF():
    def __init__(self, dynamics_model, measurement_model, n=3):
    ...
    def S_XY(self, sigma_pts, sigma_pts_meas):
        ...
        cov_xy = self.w0*np.outer(sigma_pts[0]-mu_x, sigma_pts_meas[0]-mu_y)
        for pt_x, pt_y in zip(sigma_pts[1:], sigma_pts_meas[1:]):
            cov_xy += self.wi*np.outer(pt_x-mu_x, pt_y-mu_y)
        return cov_xy
    ...
    def update(self, gauss_state, y):
        ...
        mu_nxt = mu+S_xy@np.linalg.solve(S_Y, y-mu_y)
