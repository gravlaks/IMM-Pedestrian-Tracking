...
filters = [
    iEKF(CV(sigma_q), sensor_model),
    iEKF(CT(sigma_a, sigma_w=sigma_w), sensor_model),
    iEKF(CA(sigma=sigma_a*10), sensor_model),
...
if data == 'synthetic':
    ...
    X, zs, switches = (np.load('../data/trajectories/x.npy'), np.load('../data/trajectories/z.npy'), np.load('../data/trajectories/switches.npy'))
    print(switches*dt)
    ...
    N = len(X)
if data == '../data/trajectories/ped_dataset':
    dt = 1/30
...
plt.grid(True)
plt.axis('equal')
plt.savefig('../plots/examples/traj.png')
...
plt.ylim([0, 1])
plt.grid(True)
plt.savefig('../plots/examples/probs.png')
